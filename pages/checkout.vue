    ...
    <div class="container mt-5">
        ...
        <b-row>
            <b-col class="px-2" lg="6" offset-lg="3" md="8" offset-md="2" sm="10" offset-sm="1">
                ...
                <template v-else>
                    <b-alert :show="paymentError" variant="danger">Ha ocurrido un error al procesar el pago, por favor intente nuevamente</b-alert>
                    <b-form @submit.prevent="submit">
                        <b-form-group label="Nombre del dueño de la tarjeta" label-for="name">

                            <b-form-input v-model="form.name"
                                            id="name"
                                            type="text"
                                            placeholder="e.g. fguzman"
                                            >
    ...
    methods: {
        async submit() {
            ...
            if (error)  {
                ...
                this.craeteSubscription(setupIntent.payment_method)
            }


        },
        async craeteSubscription(token) {
            try {
                const respose = await this.$axios.$post('/api/subscriptions/create', {
                    plan: this.plan,
                    token
                });
                this.$auth.fetchUser();
